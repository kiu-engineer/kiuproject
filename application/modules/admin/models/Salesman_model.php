...
class Salesman_model extends CI_Model {
    ...
    {
        return $this->db->order_by('name', 'ASC')->get('salesman')->result();
    }
    ...
    public function get_all_orders($limit, $start)
    {
        $orders = $this->db->query("
            SELECT o.id, o.order_number, o.order_date, o.order_status, o.payment_method, o.total_price, o.total_items, c.name AS salesman, cu.name AS customer
            FROM orders o
            LEFT JOIN salesmans c
                ON c.id = o.salesman_id
            JOIN customers cu
    ...
    public function add_salesman(Array $data)
    {
    ...
    public function salesman_data($id)
    {
        return $this->db->where('id', $id)->get('salesman')->row();
    }

    public function edit_salesman($id, $data)
    {
        return $this->db->where('id', $id)->update('salesman', $data);
    }

    public function delete_salesman($id)
    {
        return $this->db->where('id', $id)->delete('salesman');
    }
}
